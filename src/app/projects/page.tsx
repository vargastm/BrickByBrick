...
export default function Buildings() {
  ...
  useEffect(() => {
    const fetchBlockchainData = async () => {
      ...
      try {
        // --- 1. Pegar o total de buildings ---
        console.log("🔍 Buscando total de buildings...")
        ...
        // CORREÇÃO AQUI: Acessando .output baseado no seu log
        // Resposta: { kind: "MethodCallResponse", output: "2" }
        const totalRaw = totalResp.data.result.output
        const totalCount = Number(totalRaw)

        console.log(`🔢 Total encontrado: ${totalCount} (Raw: ${totalRaw})`)

        if (isNaN(totalCount) || totalCount <= 0) {
          console.warn("⚠️ Nenhum building encontrado.")
          setBuildings([])
          return
        }

        // --- 2. Loop para pegar detalhes ---
        const promises = []
        // Assumindo IDs sequenciais começando em 1.
        // SE O SEU CONTRATO COMEÇAR EM 0, MUDE PARA: let i = 0; i < totalCount; i++
        // Tentarei começar do 1 baseado no padrão comum, se falhar tente o 0.
        ...
        const formattedBuildings: Building[] = results.map((res: any) => {
          // O MultiBaas retorna os valores em um array dentro de output
          // Ordem baseada na sua lista:
          // [0]id, [1]name, [2]metadataURI, [3]developer, [4]oracle, [5]tokenContract, 
          // [6]status, [7]totalMilestones, [8]currentMilestone, [9]exists, 
          // [10]featured, [11]description, [12]location
          ...
          const progressCalc = totalMilestones > 0 ? Math.round((currentMilestone / totalMilestones) * 100) : 0
          ...
          return {
            ...
            // Dados Mockados (não presentes no contrato)
            category: "Comercial", 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-50 via-white to-zinc-50 dark:from-[#0f101a] dark:via-zinc-950 dark:to-[#0f101a]">
      ...
      {/* Seção de Erro de Debug (Remova em produção se quiser) */}
      {debugError && (
        <div className="w-full bg-red-500 text-white p-2 text-center text-sm font-bold">
          DEBUG: {debugError}
        </div>
      )}
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-1 items-center gap-4">
              <div className="relative max-w-md flex-1">
                <svg className="absolute top-1/2 left-3 h-5 w-5 -translate-y-1/2 text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                <input
                  type="text"
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
              {isLoading ? 'Loading...' : `${filteredBuildings.length} tokenized projects`}
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
           <div className="flex h-64 items-center justify-center text-zinc-500">
             <div className="text-center">
               <div className="h-8 w-8 animate-spin rounded-full border-4 border-zinc-300 border-t-black mx-auto mb-4"></div>
                    ...
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-zinc-600 dark:text-zinc-400">
                        <span className="font-medium">
                          {building.milestonesCompleted}/{building.totalMilestones} Milestones
                        </span>
                      </div>
                    </div>
